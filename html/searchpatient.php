...
<body>
		...
		<div class="floating-box" id="centerTop">
			...
			<form method="post" action="https://blackrain-technologies.com/searchPatient.php">
				...
				<input type="hidden" name="testing" value="test"  id="wewew">
		...
		if(isset($_POST['searchAll'])){
			...
			$query = "SELECT Fname, Lname, DOB, Sex, PatientID FROM patient";
			...
			if($pid=='' && $f_name=='' && $l_name==''){
				echo'<div class="floating-box">Please enter in one of the following: Patient ID or Patients first and last name.</div>' . filter_var($_POST['testing']);
			}
			if($f_name != '') {
				$query = "SELECT Fname, Lname, DOB, Sex FROM patient WHERE Fname = ". $f_name;
			}
			if($pid != ''){
